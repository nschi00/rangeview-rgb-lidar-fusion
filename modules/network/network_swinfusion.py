...
class CRSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if resi_connection == '1conv':
            ...
            # to save parameters and memory
            self.conv_A = nn.Sequential(nn.Conv2d(dim, dim // 4, 3, 1, 1),
                                        nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                        nn.Conv2d(dim // 4, dim // 4, 1, 1, 0),
                                        nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                        nn.Conv2d(dim // 4, dim, 3, 1, 1))
            self.conv_B = nn.Sequential(nn.Conv2d(dim, dim // 4, 3, 1, 1),
                                        nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                        nn.Conv2d(dim // 4, dim // 4, 1, 1, 0),
                                        nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                        nn.Conv2d(dim // 4, dim, 3, 1, 1))
...
class SwinFusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=1,
        ...
        # stochastic depth
        dpr_Ex = [x.item() for x in torch.linspace(0, drop_path_rate,
                                                   sum(Ex_depths))]  # stochastic depth decay rule
        ...
        for i_layer in range(self.Ex_num_layers):
            layer = RSTB(dim=embed_dim,
                         ...
                         drop_path=dpr_Ex[sum(Ex_depths[:i_layer]):sum(
                             Ex_depths[:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
                         img_size=img_size,
        ...
        for i_layer in range(self.Fusion_num_layers):
            layer = CRSTB(dim=embed_dim,
                          ...
                          drop_path=dpr_Fusion[sum(Fusion_depths[:i_layer]):sum(
                              Fusion_depths[:i_layer + 1])],  # no impact on SR results
                          norm_layer=norm_layer,
                          downsample=None,
                          use_checkpoint=use_checkpoint,
                          img_size=img_size,
        ...
        for i_layer in range(self.Re_num_layers):
            layer = RSTB(dim=embed_dim,
                         ...
                         qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop_rate, attn_drop=attn_drop_rate,
                         drop_path=dpr_Re[sum(Re_depths[:i_layer]):sum(
                             Re_depths[:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
                         img_size=img_size,
        ...
        if resi_connection == '1conv':
            self.conv_after_body_Ex_A = nn.Conv2d(
                embed_dim, embed_dim, 1, 1, 1)
            self.conv_after_body_Ex_A = nn.Conv2d(
                embed_dim, embed_dim, 1, 1, 1)
            self.conv_after_body_Fusion = nn.Conv2d(
                2 * embed_dim, embed_dim, 1, 1, 1)
            self.conv_after_body_Re = nn.Conv2d(embed_dim, embed_dim, 1, 1, 1)
...
if __name__ == '__main__':
    height = 64
    width = 512
    model = SwinFusion(img_size=(height, width),
                       patch_size=1,
                       in_chans=128,
                       embed_dim=128,
                       window_size=8,
