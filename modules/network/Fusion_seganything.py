...
class Fusion(nn.Module):
    ...
    def __init__(self, nclasses, full_self_attn=False):
        ...
        self.image_encoder_img_size = 1024
        self.pixel_mean = torch.Tensor([123.675, 116.28, 103.53]).view(-1, 1, 1)
        self.pixel_std = torch.Tensor([58.395, 57.12, 57.375]).view(-1, 1, 1)


        """RGB Backbone"""
        self.rgb_backbone = sam_model_registry["vit_b"](checkpoint="sam_vit_b_01ec64.pth")
        self.rgb_backbone = self.rgb_backbone.image_encoder
        ...
        self.fusion_layer = SwinFusion(upscale=upscale, img_size=(height, width), Fusion_depths=self.Fusion_depths,
                    window_size=window_size, embed_dim=inplanes, Fusion_num_heads=self.Fusion_num_heads,
                    Re_num_heads=[8], Re_depths=[4], mlp_ratio=2, upsampler='', in_chans=inplanes, ape=True,
                    drop_path_rate=0.)
    ...
    def forward(self, lidar, rgb):
        ...
        x_rgb = self.rgb_backbone(self.preprocess(rgb))
        x_rgb = F.interpolate(x_rgb, x_lidar_fusion.shape[-2:], mode="bilinear", align_corners=False)
    ...
    def preprocess(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Pad
        h, w = x.shape[-2:]
        padh = self.image_encoder_img_size - h
        padw = self.image_encoder_img_size - w
